...
-define(FILEPATH, "src/mond_http.erl").
...
?DOC(
    " Convert a HTTP scheme to a string\n"
    "\n"
    " # Examples\n"
    "\n"
    " ```gleam\n"
...
parse_multipart_body(Data, Boundary) ->
    Boundary@1 = mond_http_runtime:identity(Boundary),
...
parse_rfc_2045_parameter_quoted_value(Header, Name, Value) ->
    case mond_http_runtime:string_pop_grapheme(Header) of
        {error, nil} ->
            {error, nil};
        ...
        {ok, {<<"\\"/utf8>>, Rest@1}} ->
            mond_http_runtime:result_try(
                mond_http_runtime:string_pop_grapheme(Rest@1),
                fun(_use0) ->
...
parse_rfc_2045_parameter(Header, Name) ->
    mond_http_runtime:result_try(
        mond_http_runtime:string_pop_grapheme(Header),
        fun(_use0) ->
            {Grapheme, Rest} = _use0,
            case Grapheme of
                <<"="/utf8>> ->
...
parse_header_value_loop(Data, Headers, Name, Value) ->
    case Data of
        ...
        <<"\r\n\r\n"/utf8, Data@2/binary>> ->
            mond_http_runtime:result_map(
                mond_http_runtime:bit_array_to_string(Value),
                fun(Value@1) ->
...
parse_header_name_loop(Data, Headers, Name) ->
    case Data of
        <<":"/utf8, Data@1/bitstring>> ->
            case mond_http_runtime:bit_array_to_string(Name) of
                {ok, Name@1} ->
